...
class Lagou_info(object):
    ...
    def get_comp_question_info(self, company_id):
        ...
        try:
            ...
            print('question_info:{0}'.format(question_info))
            question_answer_list = soup.find('ul', attrs={'id': 'question-answer-list'}).find_all('li',
                                                                            class_='items-question clearfix')
            question_list = []
            for i in question_answer_list:
                info = []
                info.append(self.http + i.find('a', class_='item-question').get('href') if i.find('a',
                                                                            class_='item-question') else None)  # 问答网址
                # print(self.http + i.find('a', class_='item-question').get('href') if i.find('a',
                #                                                             class_='item-question') else None)

                info.append(i.find('a', class_='item-question').get_text() if i.find('a',
                                                                            class_='item-question') else None)  # 问题
                # print(i.find('a', class_='item-question').get_text() if i.find('a',
                #                                                             class_='item-question') else None)

                info.append(i.find('textarea', class_="answer-html").get_text() if i.find('textarea',
                                                                            class_="answer-html") else None)  # 回答
                # print(i.find('textarea', class_="answer-html").get_text() if i.find('textarea',
                #                                                             class_="answer-html") else None)
...
if __name__ == '__main__':
    ...
    print('{0},公司问答: {1}'.format(comp_name, lagou.get_comp_question_info(company_id)))
